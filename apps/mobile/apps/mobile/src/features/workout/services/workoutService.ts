...
const SAMPLE_DATA_VERSION = '2026-05-17-active-workout-v2';
...
async function ensureDemoExercise(data: {
  ...
  const timestamp = now();
  const assignMetadata = (record: any) => {
    ...
    record.secondaryMusclesJson = JSON.stringify(data.secondaryMuscles || []);
    record.equipmentType = data.equipment;
    record.movementPattern = data.movementPattern || record.movementPattern || '';
    record.isAiTracked = !!data.isAiTracked || isAiTrackedByName(data.name);
    record.aiExerciseClass = data.aiExerciseClass || (record.isAiTracked ? data.name.toLowerCase().replace(/[^a-z0-9]+/g, '_') : '');
    record.isBodyweight = !!data.isBodyweight || data.equipment.toLowerCase() === 'bodyweight' || data.equipment.toLowerCase() === 'assisted';
    record.isCustom = true;
    record.thumbnailUrl = data.thumbnailUrl || record.thumbnailUrl || '';
    record.demoVideoUrl = data.demoVideoUrl || record.demoVideoUrl || '';
...
async function seedDemoWorkout() {
  ...
  const exerciseSeed = {
    bench: await ensureDemoExercise({
      ...
      notes: 'AI-tracked barbell press used for upper-body strength. Keep the shoulder blades stable and lower the bar under control.',
      thumbnailUrl: 'https://placehold.co/600x400/111111/E9FF21/png?text=Bench+Press',
      isAiTracked: true,
      aiExerciseClass: 'bench_press',
      isBodyweight: false,
    }),
    squat: await ensureDemoExercise({
      ...
      notes: 'AI-tracked lower-body strength movement. Keep the knees tracking with the toes and maintain a stable torso.',
      thumbnailUrl: 'https://placehold.co/600x400/111111/E9FF21/png?text=Squat',
      isAiTracked: true,
      aiExerciseClass: 'squat',
      isBodyweight: false,
    }),
    pullUp: await ensureDemoExercise({
      exerciseId: 'demo-pull-up-assisted',
      name: 'Pull Up',
      muscleGroup: 'Back',
      ...
      notes: 'AI-tracked bodyweight pulling movement. Use negative load values when logging machine assistance.',
      thumbnailUrl: 'https://placehold.co/600x400/111111/E9FF21/png?text=Pull+Up',
      isAiTracked: true,
      aiExerciseClass: 'pull_up',
      isBodyweight: true,
    }),
    pushUp: await ensureDemoExercise({
      exerciseId: 'demo-push-up-bodyweight',
      name: 'Push Up',
      muscleGroup: 'Chest',
      equipment: 'Bodyweight',
      primaryMuscle: 'Chest',
      ...
      movementPattern: 'Horizontal Push',
      notes: 'AI-tracked bodyweight pushing movement used for rep counting and form monitoring.',
      thumbnailUrl: 'https://placehold.co/600x400/111111/E9FF21/png?text=Push+Up',
      isAiTracked: true,
      aiExerciseClass: 'push_up',
      isBodyweight: true,
    }),
    lateralRaise: await ensureDemoExercise({
      exerciseId: 'demo-lateral-raise-dumbbell',
      name: 'Lateral Raise',
      muscleGroup: 'Shoulders',
      equipment: 'Dumbbell',
      primaryMuscle: 'Shoulders',
      secondaryMuscles: ['Side Delt'],
      movementPattern: 'Isolation',
      notes: 'Manual accessory lift for shoulder hypertrophy. Use controlled movement and avoid swinging.',
      thumbnailUrl: 'https://placehold.co/600x400/111111/E9FF21/png?text=Lateral+Raise',
      isAiTracked: false,
      isBodyweight: false,
    }),
    sideBend: await ensureDemoExercise({
      exerciseId: 'demo-45-side-bend-bodyweight',
      name: '45° Side Bend',
      muscleGroup: 'Core',
      equipment: 'Bodyweight',
      ...
      notes: 'Manual core accessory exercise. Track reps per set and control the lowering phase.',
      thumbnailUrl: 'https://placehold.co/600x400/111111/E9FF21/png?text=Side+Bend',
      isAiTracked: false,
      isBodyweight: true,
    }),
  };
  ...
  const routine = await routines.create((record: any) => {
    ...
    record.notes = `CoreSet aligned local demo routine • ${SAMPLE_DATA_VERSION}`;
  ...
  const routineExerciseInputs = [
    {
      exercise: exerciseSeed.bench,
      sortOrder: 1,
      targetSets: 3,
      targetReps: 8,
      weight: 65,
      rest: 180,
      note: 'Focus on stable setup and controlled lowering.',
      focus: 'previous' as FocusMetric,
      sets: [
        { order: 1, type: 'warmup', prevW: 45, prevR: 8, w: 45, r: 8, rpe: 0, rest: 90, done: true },
        { order: 2, type: 'normal', prevW: 65, prevR: 4, w: 65, r: 5, rpe: 7, rest: 180, done: false },
        { order: 3, type: 'failure', prevW: 65, prevR: 3, w: 70, r: 10, rpe: 8, rest: 180, done: false },
      ],
    },
    {
      exercise: exerciseSeed.lateralRaise,
      sortOrder: 2,
      targetSets: 3,
      targetReps: 12,
      weight: 10,
      rest: 90,
      note: 'Accessory lift; keep the elbows slightly bent.',
      focus: 'total_reps' as FocusMetric,
      sets: [
        { order: 1, type: 'normal', prevW: 10, prevR: 12, w: 10, r: 12, rpe: 7, rest: 90, done: false },
        { order: 2, type: 'normal', prevW: 10, prevR: 10, w: 10, r: 10, rpe: 8, rest: 90, done: false },
      ],
    },
    {
      exercise: exerciseSeed.squat,
      sortOrder: 3,
      targetSets: 4,
      targetReps: 10,
      weight: 70,
      rest: 180,
      note: 'AI-tracked movement included for demo of camera-enabled exercises.',
      focus: 'total_volume' as FocusMetric,
      sets: [
        { order: 1, type: 'warmup', prevW: 30, prevR: 5, w: 30, r: 5, rpe: 0, rest: 90, done: false },
        { order: 2, type: 'normal', prevW: 65, prevR: 4, w: 70, r: 10, rpe: 8, rest: 180, done: false },
        { order: 3, type: 'normal', prevW: 65, prevR: 3, w: 70, r: 10, rpe: 8, rest: 180, done: false },
        { order: 4, type: 'drop', prevW: 60, prevR: 2, w: 60, r: 8, rpe: 8, rest: 90, done: false },
      ],
    },
  ];
  ...
  for (const item of routineExerciseInputs) {
    const routineExercise = await routineExercises.create((record: any) => {
      ...
      record.targetRepsMin = Math.max(1, item.targetReps - 2);
      record.targetRepsMax = item.targetReps + 2;
      record.targetRpe = 8;
  ...
  const session = await sessions.create((record: any) => {
    ...
    record.notes = `sample_data_version=${SAMPLE_DATA_VERSION}`;
    record.deletedAt = 0;
    record.createdAt = timestamp;
    record.updatedAt = timestamp;
  }) as any;

  for (const item of routineExerciseRecords) {
    for (const input of item.sets) {
      await workoutSets.create((record: any) => {
        record.remoteSetId = '';
        record.sessionId = session.id;
        record.exerciseId = item.exercise.id;
        record.routineExerciseId = item.routineExercise.id;
        record.setOrder = input.order;
        record.setType = input.type;
        record.setNumber = input.order;
        record.isWarmup = input.type === 'warmup';
        record.logMode = item.exercise.isAiTracked ? 'ai' : 'manual';
        ...
        record.modelConfidenceAvg = item.exercise.isAiTracked ? 0.85 : 0;
...
async function softDeleteLocalDemoSession(session: any) {
  const timestamp = now();
  const routineId = session.routineId;
  const workoutSets = await database.collections
    .get('workout_sets')
    .query(Q.where('session_id', session.id), isLiveRecordQuery())
    .fetch();
  ...
  if (routineId) {
    const routineExercises = await database.collections
      .get('routine_exercises')
      .query(Q.where('routine_id', routineId), isLiveRecordQuery())
      .fetch();

    for (const routineExercise of routineExercises as any[]) {
    ...
    try {
      const routine = await database.collections.get('routines').find(routineId) as any;
      await routine.update((record: any) => {
        record.deletedAt = timestamp;
        record.updatedAt = timestamp;
      });
    } catch {
      // Routine may already be deleted; ignore.
    }
  }
}
...
export async function getOrCreateActiveWorkoutSession(): Promise<WorkoutSessionVM> {
  ...
  const firstActive = activeSessions[0] as any | undefined;
  const isOutdatedLocalDemo =
    firstActive &&
    !firstActive.remoteSessionId &&
    ['Upper Body Day', 'AI Strength Starter'].includes(String(firstActive.name || '')) &&
    !String(firstActive.notes || '').includes(SAMPLE_DATA_VERSION);

  if (isOutdatedLocalDemo) {
    await database.write(async () => {
      await softDeleteLocalDemoSession(firstActive);
    });
